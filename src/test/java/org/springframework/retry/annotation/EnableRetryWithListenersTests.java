...
public class EnableRetryWithListenersTests {
	...
	@Test
	public void overrideWithNonExistentListener() {
		AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext(
				TestConfigurationNonExistentListeners.class);
		ServiceWithNonExistentListeners service = context.getBean(ServiceWithNonExistentListeners.class);
		service.service();
		assertThat(context.getBean(TestConfigurationNonExistentListeners.class).count).isEqualTo(0);
		context.close();
	}
	...
	@Configuration
	@EnableRetry(proxyTargetClass = true)
	protected static class TestConfigurationNonExistentListeners {

		private int count = 0;

		@Bean
		public ServiceWithNonExistentListeners service() {
			return new ServiceWithNonExistentListeners();
		}
	...
	protected static class ServiceWithNonExistentListeners {

		private int count = 0;

		@Retryable(backoff = @Backoff(delay = 1000), listeners = "non-existent listener")
		public void service() {
			if (count++ < 2) {
				throw new RuntimeException("Planned");
			}
